...
export default function Roadmap() {
  return (
    <section className="container mx-auto px-4 md:px-6 py-24 max-w-6xl" aria-label="Roadmap PequeChat">
  <SectionHeading
        ...
        <div className="space-y-12">
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                {/* Timeline icon */}
                <div className={`absolute left-0 top-2 flex h-16 w-16 items-center justify-center rounded-full ${config.bgColor} shadow-lg`}>
                  <Icon className="w-7 h-7 text-white" />
                </div>
                
                {/* Content card */}
                <div className={`rounded-2xl ${config.bgLight} border ${config.borderColor} p-8 hover:shadow-lg transition-all duration-300`}>
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                    ...
                    <div className="flex flex-col items-start md:items-end gap-2">
                      <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border ${config.borderColor} ${config.bgLight}`}>
                  ...
                  {item.status === "in-progress" && (
                    <div className="mt-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-azul2">Progreso</span>
